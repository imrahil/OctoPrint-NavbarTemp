$(function () {
    function NavbarTempViewModel(parameters) {
        ...
        self.formatBarTemperature = function (toolName, actual, target) {
            ...
            // Add fahrenheit
            if (
                OctoPrint.coreui.viewmodels.settingsViewModel.appearance_showFahrenheitAlso()
            ) {
        ...
        self.onDataUpdaterPluginMessage = function (plugin, data) {
            ...
            if (data.soctemp) {
                ...
                if (
                    ...
                    fahrenheit =
                        " (" + _.sprintf("%.1f&deg;F", data.soctemp * (9 / 5) + 32) + ")";
                }
                if (self.settings.makeMoreRoom() == false) {
                    self.socTemp(
                        self.settings.soc_name() +
                            _.sprintf(": %.1f&deg;C", data.soctemp) +
                            fahrenheit
                    );
                } else {
                    self.socTemp(
            ...
            if (data.cmd_name) {
                self.custCmd(
                    _.sprintf(" %s: ", data.cmd_name) + _.sprintf("%s", data.cmd_result)
                );
            }
        };
